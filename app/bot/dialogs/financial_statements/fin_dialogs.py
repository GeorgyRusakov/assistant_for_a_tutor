...
finpreview = Window(
    ...
    Column(
        ...
        SwitchTo(Const('Отчетность за месяц'), id='sw2', state=states.Finance.fin_report_month),
        SwitchTo(Const('Аналитика по месяцам'), id='sw3', state=states.Finance.fin_report_months),
        SwitchTo(Const('Аналитика за все время'), id='sw4', state=states.Finance.fin_all_time_report),
        MAIN_MENU_BUTTON,
    ),
...
fin_report_month = Window(
    Format('Финансовый отчет за текущий ***месяц***: \n\n'
           '***Итоговый результат\: {sum_total_month} руб\.*** \n'),
    Format('***Общая статистика\:*** \n' \
           '{report_table}'),
    SwitchTo(Const('⬅️Назад'), id='st', state=states.Finance.finpreview),
    MAIN_MENU_BUTTON,
